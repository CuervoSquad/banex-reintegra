...
import {
  buildDemoQrPayment, buildLocalCashbackStream, buildMonthlyBatchQrPayment,
  buildQrReportRows, calculateStreamSnapshot, cashbackLevels, formatBobCurrency,
  generateRandomQrPaymentsUntilTarget, getCashbackLevel, sumPaymentsBs,
  type CashbackLevelId, type DemoQrPayment,
} from './streamCashback';
...
export function useStreamCashback() {
  ...
  async function createQrPayment() {
    ...
    try {
      ...
      try { created = await cashbackService.createFromQrPayment(payload); }
      catch { created = buildLocalCashbackStream(payload); }
      setMonthlyPayments((c) => [...c, { id: created.qr_payment_id, merchant_name: created.merchant_name, payment_amount: created.payment_amount }]);
      setStream(created);
      setNow(Date.now());
  ...
  async function claimCashback() {
    ...
    try {
      if (stream.id.startsWith('local-')) {
        setStream({ ...stream, claimed_amount: String(snapshot.accumulatedBs), claimable_amount: '0.000000', server_time: new Date().toISOString() });
        return;
      }
  ...
  const reportRows = useMemo(() => buildQrReportRows(snapshot, { levelId: selectedLevelId, monthlyConsumptionBs }), [monthlyConsumptionBs, selectedLevelId, snapshot]);
  const formattedAccumulated = useMemo(() => formatBobCurrency(snapshot.accumulatedBs), [snapshot.accumulatedBs]);

  return {
    stream, loading, claiming, creatingQr, generatingMonth,
    selectedLevel, selectedLevelId, cashbackLevels, monthlyPayments,
    monthlyConsumptionBs, monthlyProgress, snapshot, reportRows, formattedAccumulated,
    selectLevel: setSelectedLevelId, createQrPayment, generateMonthlyQrPayments, claimCashback,
  };
}
