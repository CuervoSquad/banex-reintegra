...
export const cashbackLevels = [
  { id: 'level-1', name: 'Nivel 1', targetBs: 600, percentage: '0.01', label: '1.0%' },
  { id: 'level-2', name: 'Nivel 2', targetBs: 1500, percentage: '0.015', label: '1.5%' },
  { id: 'level-3', name: 'Nivel 3', targetBs: 3000, percentage: '0.02', label: '2.0%' },
] as const satisfies ReadonlyArray<{ id: CashbackLevelId; name: string; targetBs: number; percentage: string; label: string }>;
...
export function getCashbackLevel(levelId: CashbackLevelId) {
  return cashbackLevels.find((l) => l.id === levelId) ?? cashbackLevels[1];
}

export function buildDemoQrPayment(prefix = 'QR', options?: { levelId?: CashbackLevelId; payment?: Pick<DemoQrPayment, 'merchant_name' | 'payment_amount'> }): QRPaymentCreate {
...
export function buildMonthlyBatchQrPayment(payments: DemoQrPayment[], levelId: CashbackLevelId): QRPaymentCreate {
  const totalBs = sumPaymentsBs(payments);
  return buildDemoQrPayment('QR-MES', { levelId, payment: { merchant_name: `${payments.length} pagos QR generados`, payment_amount: totalBs.toFixed(2) } });
}
...
export function generateRandomQrPaymentsUntilTarget(levelId: CashbackLevelId): DemoQrPayment[] {
  ...
  while (total < level.targetBs) {
    ...
    payments.push({ id: `QR-${Date.now()}-${payments.length + 1}`, merchant_name: payment.merchant_name, payment_amount: payment.payment_amount });
...
export function sumPaymentsBs(payments: DemoQrPayment[]) {
  return payments.reduce((t, p) => t + Number(p.payment_amount), 0);
}
...
export function calculateStreamSnapshot(stream: CashbackStream | null, now: number) {
  ...
  const accumulatedBs = Math.min(cashbackTotalBs, Number(stream?.streamed_amount ?? 0) + elapsedSinceFetch * ratePerSecond);
  const claimableBs = Math.max(0, Number(stream?.claimable_amount ?? 0) + elapsedSinceFetch * ratePerSecond);
  return {
    startsAt, endsAt, ratePerSecond, paymentAmountBs, cashbackTotalBs,
    ...
    accumulatedBs, accumulatedUsdt: accumulatedBs / EXCHANGE_RATE_BOB, claimableBs,
    progress: Math.min(100, ((now - startsAt) / Math.max(1, endsAt - startsAt)) * 100),
  };
}

export function buildQrReportRows(snapshot: ReturnType<typeof calculateStreamSnapshot>, options?: { levelId?: CashbackLevelId; monthlyConsumptionBs?: number }) {
  const level = getCashbackLevel(options?.levelId ?? 'level-2');
  ...
  return [
    ...
    { label: 'Reintegro total', value: `${snapshot.cashbackTotalUsdt.toFixed(4)} USDT · ${formatBob(snapshot.cashbackTotalBs)}` },
  ];
}
